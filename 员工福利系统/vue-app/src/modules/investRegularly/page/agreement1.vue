...
<div>
    <div v-if="queryPower.xy!=='0'">
        ...
        <ul class="activity-rules-list">
          <li>甲方：<b>{{enterpriseName}}</b></li>
          <li>乙方：<b>{{userName}}({{idCardMark(idCardNo)}})</b></li>
          <li>鉴于，甲方因公司发展及生产经营之需要，同时，为提高员工参与公司发展的积极性，甲乙双方本着平等自愿的原则，经协商一致达成如下协议，以资双方恪守：</li>
          <li>一、乙方自愿向甲方提供借款人民币<b>{{prod_money_inc}}</b>元（大写:【 <b>{{DX(prod_money_inc)}}</b>】）。乙方将相应借款资金支付给甲方集资平台时，则视为已向甲方提供借款。</li>
          ...
          <li>四、本协议项下的借款期限为  <b>{{prodTerm}}</b><span v-if="fixedTerm==2" >个月</span> <span v-if="fixedTerm==1" >天</span>   ，自甲方收到乙方借款之日起算。借款年利率为 <b>{{prodRate}}</b> ，双方按规定时间进行资金（含本金、利息）结算。</li>
          ...
          <li>乙方：<b>{{userName}}({{idCardMark(idCardNo)}})</b></li>
          <li>&nbsp;</li>
          <li>签署日期：<b>{{transDate(orderDate)}}</b></li>
        </ul>
    </div>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            this.fixedTerm = this.$route.params.fixedTerm;
            this.prod_money_inc = this.$route.params.prod_money_inc;
            this.prodRate = this.$route.params.prodRate;
            this.prodTerm = this.$route.params.prodTerm;
            ...
            util.axiosPost('/eebpc/wealthCenter/getEnterpriseAndEmployeeName').then(resp=>{
                ...
                if(this.queryPower.xy==='0'){
                    ...
                    this.url = `${url.etsUrl}eebh5/assets/xieyi/${resp.data.instanceId}.html?flag=1`;
                }
                this.url += encodeURI(`&enterpriseName=${this.enterpriseName}&userName=${this.userName}&idCardNo=${this.idCardMark(this.idCardNo)}&prod_money_inc=${this.prod_money_inc&&this.prod_money_inc!==''?this.prod_money_inc:'0'}&prod_money_inc_dx=${this.DX(this.prod_money_inc)}&prodTerm=${this.prodTerm}&fixedTerm=${this.fixedTerm}&prodRate=${this.prodRate}&orderDate=${this.transDate(this.orderDate)}&interestsDate=${this.transDate(this.interestsDate)}`);
            });
        },
        mounted(){
            //if(!this.queryPower.xy!=='0'){
